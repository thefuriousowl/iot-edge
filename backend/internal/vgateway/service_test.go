...
func TestNewVGatewayServiceRequiresDependencies(t *testing.T) {
	...
	driver, err := modbus.NewModbusTCPDriver(
...
func TestNewVGatewayServiceInitializesIsolatedRuntimeRegistry(t *testing.T) {
	...
	if first.drivers[VGatewayTypeModbusTCP] != driver {
	...
	delete(drivers, VGatewayTypeModbusTCP)
	if _, exists := first.drivers[VGatewayTypeModbusTCP]; !exists {
...
func TestNewVGatewayServiceClockUsesCurrentTime(t *testing.T) {
	...
	service, err := NewVGatewayService(
		...
		GatewayDriverRegistry{VGatewayTypeModbusTCP: driver},
	)
	if err != nil {
...
func mustRawConfig(t *testing.T, config any) VGatewayConfig {
	t.Helper()
...
func rawConfigPointer(t *testing.T, config any) *VGatewayConfig {
	t.Helper()
	raw := mustRawConfig(t, config)
	return &raw
}

func rawConfigValue(config any) VGatewayConfig {
	raw, err := json.Marshal(config)
	if err != nil {
		panic(err)
	}
...
var _ VGatewayRepository = (*stubVGatewayRepository)(nil)

func (r *stubVGatewayRepository) Create(ctx context.Context, gateway *VGateway) error {
	if r.createFunc != nil {
...
func (r *stubVGatewayRepository) FindByID(ctx context.Context, id uuid.UUID) (*VGateway, error) {
...
func (r *stubVGatewayRepository) Update(ctx context.Context, gateway *VGateway) error {
	if r.updateFunc != nil {
