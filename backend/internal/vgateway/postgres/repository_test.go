...
func TestVGatewayRepository_CRUDIntegration(t *testing.T) {
	...
	found.Enabled = true
	found.Type = vgateway.VGatewayType("mqtt")
	...
	if updated.Type != vgateway.VGatewayTypeModbusTCP {
	...
	if _, err := repository.FindByID(ctx, gateway.ID); !errors.Is(err, vgateway.ErrVGatewayNotFound) {
		t.Errorf("FindByID() after delete error = %v, want vgateway.ErrVGatewayNotFound", err)
	}
}
...
func TestVGatewayRepository_ListIntegration(t *testing.T) {
	...
	gateways, total, err := repository.List(ctx, vgateway.VGatewayListOptions{
		Limit:  2,
		Offset: 1,
	})
	if err != nil {
	...
	disabled := false
	disabledGateways, disabledTotal, err := repository.List(ctx, vgateway.VGatewayListOptions{
		Enabled: &disabled,
	})
	if err != nil {
	...
	unsupportedType := vgateway.VGatewayType("mqtt")
	emptyGateways, emptyTotal, err := repository.List(ctx, vgateway.VGatewayListOptions{
...
func TestVGatewayRepository_ErrorMappingIntegration(t *testing.T) {
	...
	if err := repository.Create(ctx, duplicate); !errors.Is(err, vgateway.ErrVGatewayNameExists) {
		t.Errorf("Create() duplicate error = %v, want vgateway.ErrVGatewayNameExists", err)
	}
	...
	if err := repository.Update(ctx, second); !errors.Is(err, vgateway.ErrVGatewayNameExists) {
		t.Errorf("Update() duplicate error = %v, want vgateway.ErrVGatewayNameExists", err)
	}

	missingID := uuid.New()
	if _, err := repository.FindByID(ctx, missingID); !errors.Is(err, vgateway.ErrVGatewayNotFound) {
		t.Errorf("FindByID() missing error = %v, want vgateway.ErrVGatewayNotFound", err)
	}
	missing := newRepositoryTestVGateway("Missing Gateway", true)
	missing.ID = missingID
	if err := repository.Update(ctx, missing); !errors.Is(err, vgateway.ErrVGatewayNotFound) {
		t.Errorf("Update() missing error = %v, want vgateway.ErrVGatewayNotFound", err)
	}
	if err := repository.Delete(ctx, missingID); !errors.Is(err, vgateway.ErrVGatewayNotFound) {
		t.Errorf("Delete() missing error = %v, want vgateway.ErrVGatewayNotFound", err)
	}
}
...
func newRepositoryTestVGateway(name string, enabled bool) *vgateway.VGateway {
	...
	return &vgateway.VGateway{
		...
		Enabled:     enabled,
		Config: mustRepositoryConfigValue(modbus.ModbusTCPConfig{
			Host:              "192.168.1.100",
			Port:              502,
			Timeout:           5_000,
			RetryCount:        3,
...
func mustRepositoryConfig(
	t *testing.T,
	config modbus.ModbusTCPConfig,
) vgateway.VGatewayConfig {
	t.Helper()
...
func decodeRepositoryModbusConfig(
	t *testing.T,
	raw vgateway.VGatewayConfig,
) modbus.ModbusTCPConfig {
	t.Helper()
	var config modbus.ModbusTCPConfig
...
func newTestVGatewayRepository(t *testing.T) (vgateway.VGatewayRepository, *gorm.DB) {
	t.Helper()
