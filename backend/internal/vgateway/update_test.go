...
func TestVGatewayServiceUpdateAppliesPartialFields(t *testing.T) {
	...
	repo := &stubVGatewayRepository{
		...
		updateFunc: func(gotCtx context.Context, gateway *VGateway) error {
			updateCalls++
			if gotCtx.Value(requestKey) != "update-1" {
	...
	got, err := service.Update(ctx, existing.ID, UpdateVGatewayInput{
		...
		Enabled: &enabled,
		Config: rawConfigPointer(t, modbus.ModbusTCPConfigInput{
	...
	if got.ID != existing.ID || got.Type != VGatewayTypeModbusTCP || !got.CreatedAt.Equal(existing.CreatedAt) {
	...
	if got.Status != VGatewayStatusStopped {
		t.Fatalf("Status = %q, want %q", got.Status, VGatewayStatusStopped)
	}
	if factoryCalls != 0 {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			service := newTestVGatewayService(t, &stubVGatewayRepository{
				...
				updateFunc: func(context.Context, *VGateway) error {
					updateCalls++
					return nil
				},
			}, nil)
...
func TestVGatewayServiceUpdateEmptyInputIsReadOnly(t *testing.T) {
	...
	if got.Status != VGatewayStatusConnected {
		t.Fatalf("Status = %q, want %q", got.Status, VGatewayStatusConnected)
	}
}
...
func TestVGatewayServiceUpdateMapsRepositoryErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			updateError: ErrVGatewayNotFound,
			wantError:   ErrVGatewayNotFound,
		},
		{
			name:        "duplicate name",
			updateError: ErrVGatewayNameExists,
...
func TestVGatewayServiceUpdatePreservesRuntimeForIdenticalConfig(t *testing.T) {
	...
	existing.Config = VGatewayConfig(`{
		"timeout":5000,
...
func TestVGatewayServiceUpdateReconcilesEnabledState(t *testing.T) {
	...
	t.Run("disable disconnects and stops runtime", func(t *testing.T) {
		...
		if got.Status != VGatewayStatusStopped || runtime.status != VGatewayStatusStopped {
	...
	t.Run("enable resets stopped runtime to disconnected", func(t *testing.T) {
		...
		runtime := &vGatewayRuntime{status: VGatewayStatusStopped}
		service := newTestVGatewayService(t, &stubVGatewayRepository{
			findByIDFunc: func(context.Context, uuid.UUID) (*VGateway, error) {
				return existing, nil
			},
		...
		if got.Status != VGatewayStatusDisconnected || runtime.status != VGatewayStatusDisconnected {
...
func TestVGatewayServiceUpdateRecordsRuntimeDisconnectFailure(t *testing.T) {
	...
	if got.Status != VGatewayStatusError || runtime.status != VGatewayStatusError {
...
func TestVGatewayServiceUpdateDoesNotInvalidateRuntimeWhenPersistenceFails(t *testing.T) {
	...
	if client.disconnectCallCount() != 0 || runtime.client != client || runtime.status != VGatewayStatusConnected {
		t.Fatal("failed persistence mutated active runtime")
	}
}

func existingVGateway() *VGateway {
...
func modbusConfigInputFrom(raw VGatewayConfig) modbus.ModbusTCPConfigInput {
	var config modbus.ModbusTCPConfig
	if err := json.Unmarshal(raw, &config); err != nil {
		panic(err)
	}
	return modbus.ModbusTCPConfigInput{
		Host:              config.Host,
		Port:              &config.Port,
...
type stubModbusClient struct {
	...
	readCalls       int
	readRequest     modbus.ReadRequest
	readErr         error
}

var _ modbus.ModbusClient = (*stubModbusClient)(nil)
...
func (c *stubModbusClient) readCallState() (int, modbus.ReadRequest) {
	c.mu.Lock()
	defer c.mu.Unlock()
