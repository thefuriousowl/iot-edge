...
import (
	...
	authcore "github.com/thefuriousowl/iot-edge/internal/auth"
)
...
func (r *routeAuthService) Setup(context.Context, string, string) (*authcore.User, error) {
	return &authcore.User{Username: "admin"}, nil
}

func (r *routeAuthService) Login(context.Context, string, string) (*authcore.TokenPair, error) {
...
func (r *routeAuthService) Refresh(context.Context, string) (*authcore.AccessTokenResult, error) {
	return &authcore.AccessTokenResult{
...
func (r *routeAuthService) ParseToken(
	tokenString string,
	expectedType string,
) (*authcore.TokenClaims, error) {
	r.receivedToken = tokenString
	r.receivedExpectedType = expectedType
	if r.parseErr != nil {
		return nil, r.parseErr
	}
	return &authcore.TokenClaims{
		Username:  "admin",
		TokenType: authcore.TokenTypeAccess,
...
func TestRegisterAuthRoutes_ProtectsMeRoute(t *testing.T) {
	tests := []struct {
		...
		{name: "missing authorization"},
		{
			...
			parseErr:      authcore.ErrInvalidToken,
		},
	}
...
func TestRegisterAuthRoutes_AuthenticatedMeReachesHandler(t *testing.T) {
	...
	if auth.receivedToken != "valid-access-token" || auth.receivedExpectedType != authcore.TokenTypeAccess {
		t.Errorf(
