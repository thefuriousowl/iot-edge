	...
	for _, scheme := range []string{"Bearer", "bearer", "BEARER"} {
		t.Run(scheme, func(t *testing.T) {
			...
			app.Get("/protected", RequireAuth(parser), func(c *fiber.Ctx) error {
				...
				gotClaims, ok = c.Locals(LocalClaims).(*auth.TokenClaims)
				if !ok {
					t.Error("LocalClaims is not *auth.TokenClaims")
				}
			...
			if parser.receivedType != auth.TokenTypeAccess {
				t.Errorf("ParseToken() expected type = %q, want %q", parser.receivedType, auth.TokenTypeAccess)
			}
			if gotUserID != userID {
...
func TestRequireAuth_InvalidClaimsReturnUnauthorized(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			parser: &fakeAccessTokenParser{claims: &auth.TokenClaims{
				Username:         "admin",
