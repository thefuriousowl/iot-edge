...
func TestUserRepository_Integration(t *testing.T) {
	...
	if _, err := repository.FindByID(ctx, uuid.New()); !errors.Is(err, auth.ErrUserNotFound) {
		t.Errorf("FindByID() missing error = %v, want ErrUserNotFound", err)
	}
	if _, err := repository.FindByUsername(ctx, "missing-user"); !errors.Is(err, auth.ErrUserNotFound) {
	...
	if err := repository.Update(ctx, &missingUser); !errors.Is(err, auth.ErrUserNotFound) {
...
func TestUserRepository_CreateInitialUser_Integration(t *testing.T) {
	...
	if err := repository.CreateInitialUser(ctx, secondUser); !errors.Is(err, auth.ErrInitialUserExists) {
...
func TestUserRepository_CreateInitialUserConcurrent_Integration(t *testing.T) {
	...
	for result := range results {
		...
		case errors.Is(result.err, auth.ErrInitialUserExists):
			alreadyExistsErrors++
		default:
