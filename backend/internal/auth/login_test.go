...
func (r *loginUserRepository) Update(ctx context.Context, user *User) error {
	r.updateCalls++
	r.receivedCtx = ctx
	r.updatedUser = user
	return r.updateErr
...
func TestLogin_MapsMissingUserToInvalidCredentials(t *testing.T) {
	users := &loginUserRepository{findErr: ErrUserNotFound}
...
func TestLogin_RejectsActiveAccountLock(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			users := &loginUserRepository{user: &User{
				ID:          uuid.New(),
				Username:    "admin",
				IsLocked:    true,
...
func newLoginTestUser(t *testing.T, auth *authService, password string) *User {
	t.Helper()
