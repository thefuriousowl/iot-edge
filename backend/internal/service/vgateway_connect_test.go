...
func TestVGatewayServiceConnectCreatesAndConnectsProtocolClient(t *testing.T) {
	...
	}, func(config protocol.ModbusTCPConfig) (protocol.ModbusClient, error) {
		factoryCalls++
	...
	if runtime.client != client || runtime.status != domain.VGatewayStatusConnected {
...
func TestVGatewayServiceConnectMapsLookupAndTypeErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			gateway: func() *domain.VGateway {
				gateway := existingVGateway()
				gateway.Type = domain.VGatewayType("mqtt")
				return gateway
			}(),
...
func TestVGatewayServiceConnectRetriesExistingClientAfterFailure(t *testing.T) {
	...
	runtime.mu.Lock()
	defer runtime.mu.Unlock()
	if runtime.status != domain.VGatewayStatusConnected || runtime.lastError != nil {
...
func TestVGatewayServiceConnectSerializesConcurrentCallsAndPublishesConnecting(t *testing.T) {
	...
	if status := service.statusForGateway(gateway.ID, true); status != domain.VGatewayStatusConnecting {
	...
	if status := service.statusForGateway(gateway.ID, true); status != domain.VGatewayStatusConnected {
...
func TestVGatewayServiceConnectDispatchesRegisteredDriverWithOwnedConfig(t *testing.T) {
	...
	gateway.Config = domain.VGatewayConfig(
