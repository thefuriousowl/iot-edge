...
func TestVGatewayServiceCreateNormalizesAndPersistsGateway(t *testing.T) {
	...
	repo := &stubVGatewayRepository{
		createFunc: func(gotCtx context.Context, gateway *domain.VGateway) error {
	...
	got, err := service.Create(ctx, CreateVGatewayInput{
		...
		Config: mustRawConfig(t, protocol.ModbusTCPConfigInput{
...
func TestVGatewayServiceCreatePreservesExplicitDisabledAndEmptyDescription(t *testing.T) {
	...
	var persisted *domain.VGateway
	repo := &stubVGatewayRepository{
		createFunc: func(_ context.Context, gateway *domain.VGateway) error {
			persisted = gateway
			return nil
		},
	}
...
func TestVGatewayServiceCreateValidatesBeforeRepository(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input: CreateVGatewayInput{
				Name: "Gateway",
				Type: domain.VGatewayType("modbus_rtu"),
				Config: mustRawConfig(t, protocol.ModbusTCPConfigInput{
					Host: "plc.example.local",
				}),
			},
...
func TestVGatewayServiceCreateMapsRepositoryErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "duplicate name",
			repoError: repository.ErrVGatewayNameExists,
...
func TestVGatewayServiceCreateDispatchesRegisteredProtocolDriver(t *testing.T) {
	...
	inputConfig := domain.VGatewayConfig(`{"broker":"mqtt.example.local"}`)
	canonicalConfig := domain.VGatewayConfig(
...
func newTestVGatewayService(
	t *testing.T,
	repo repository.VGatewayRepository,
	factory protocol.ModbusClientFactory,
) *vGatewayService {
	t.Helper()
