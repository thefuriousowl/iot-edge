...
func TestVGatewayServiceListAppliesDefaultsAndProjectsStatuses(t *testing.T) {
	...
	var gotOptions repository.VGatewayListOptions
	repo := &stubVGatewayRepository{
		listFunc: func(gotCtx context.Context, options repository.VGatewayListOptions) ([]domain.VGateway, int64, error) {
	...
	if got.Data[0].ID != connectedID || got.Data[0].Status != domain.VGatewayStatusConnected {
		t.Fatalf("first view = %#v, want connected gateway", got.Data[0])
	}
	if got.Data[1].ID != disabledID || got.Data[1].Status != domain.VGatewayStatusStopped {
...
func TestVGatewayServiceListForwardsFiltersAndPagination(t *testing.T) {
	...
	gatewayType := domain.VGatewayTypeModbusTCP
	enabled := false
	var gotOptions repository.VGatewayListOptions
	service := newTestVGatewayService(t, &stubVGatewayRepository{
		listFunc: func(_ context.Context, options repository.VGatewayListOptions) ([]domain.VGateway, int64, error) {
			gotOptions = options
...
func TestVGatewayServiceListCapsPerPage(t *testing.T) {
	...
	service := newTestVGatewayService(t, &stubVGatewayRepository{
		listFunc: func(_ context.Context, options repository.VGatewayListOptions) ([]domain.VGateway, int64, error) {
			gotOptions = options
			return []domain.VGateway{}, 250, nil
		},
	}, nil)
...
func TestVGatewayServiceListRejectsUnsupportedTypeBeforeRepository(t *testing.T) {
	...
	unsupported := domain.VGatewayType("modbus_rtu")
	repositoryCalls := 0
	service := newTestVGatewayService(t, &stubVGatewayRepository{
