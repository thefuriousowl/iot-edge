	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			service := newTestVGatewayService(t, &stubVGatewayRepository{
				findByIDFunc: func(gotCtx context.Context, gotID uuid.UUID) (*domain.VGateway, error) {
...
func TestVGatewayServiceStatusObservesConnectingWithoutWaitingForLifecycle(t *testing.T) {
	...
	case result := <-statusResult:
		...
		if result.Status != domain.VGatewayStatusConnecting {
			close(connectRelease)
			<-connectResult
	...
	if connected.Status != domain.VGatewayStatusConnected || connected.ConnectedAt == nil {
...
func TestVGatewayServiceStatusReturnsCoherentConcurrentSnapshots(t *testing.T) {
	...
	for range iterations {
		...
		case 2:
			if got.Status != domain.VGatewayStatusError ||
				got.ConnectedAt != nil ||
