...
func TestVGatewayServiceTestConnectionUsesTemporaryClientAndOptionalProbe(t *testing.T) {
	...
	wantRequest := protocol.ReadRequest{
		UnitID:       7,
		FunctionCode: protocol.FunctionReadHoldingRegisters,
		Address:      0,
		Quantity:     1,
	}
...
func TestVGatewayServiceTestConnectionDoesNotMutateActiveRuntime(t *testing.T) {
	...
	if runtime.status != domain.VGatewayStatusConnected ||
...
func TestVGatewayServiceTestConnectionMapsLookupTypeAndProbeErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			driver := tt.driver
			if driver == nil {
				modbusDriver, err := protocol.NewModbusTCPDriver(
					func(protocol.ModbusTCPConfig) (protocol.ModbusClient, error) {
						return &stubModbusClient{}, nil
					},
				)
