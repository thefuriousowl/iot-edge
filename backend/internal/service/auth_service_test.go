...
func (r *setupUserRepository) CreateInitialUser(ctx context.Context, user *domain.User) error {
	r.calls++
	r.receivedCtx = ctx
	r.receivedUser = user
	return r.err
...
func TestSetup_MapsInitialUserExistsError(t *testing.T) {
	users := &setupUserRepository{err: repository.ErrInitialUserExists}
	auth, err := NewAuthService(users, validAuthConfig())
	if err != nil {
		t.Fatalf("NewAuthService() error: %v", err)
	}
...
func validAuthConfig() *config.Config {
