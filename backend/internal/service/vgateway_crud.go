...
func (s *vGatewayService) Create(
	...
	if err := s.gateways.Create(ctx, gateway); err != nil {
		if errors.Is(err, repository.ErrVGatewayNameExists) {
...
func (s *vGatewayService) List(
	...
	gateways, total, err := s.gateways.List(
		ctx,
		repository.VGatewayListOptions{
			Type:    input.Type,
...
func cloneVGatewayConfig(config domain.VGatewayConfig) domain.VGatewayConfig {
	return append(domain.VGatewayConfig(nil), config...)
}

func equalVGatewayConfig(left, right domain.VGatewayConfig) bool {
	var leftValue any
	var rightValue any
