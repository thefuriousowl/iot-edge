...
type fakeRefreshService struct {
	service.AuthService
	result               *service.AccessTokenResult
	err                  error
	receivedCtx          context.Context
...
func (f *fakeRefreshService) Refresh(
	ctx context.Context,
	refreshToken string,
) (*service.AccessTokenResult, error) {
	f.calls++
	f.receivedCtx = ctx
	f.receivedRefreshToken = refreshToken
...
func TestAuthHandlerSetup_MapsServiceErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			serviceErr: service.ErrPasswordRequirements,
		...
		},
		{
			...
			serviceErr: service.ErrSetupAlreadyCompleted,
			wantStatus: fiber.StatusBadRequest,
			wantCode:   "AUTH008",
...
func TestAuthHandlerLogin_MapsServiceErrors(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "account locked",
			serviceErr: service.ErrAccountLocked,
			wantStatus: fiber.StatusUnauthorized,
			wantCode:   "AUTH002",
...
func TestAuthHandlerRefresh_ReturnsNewAccessToken(t *testing.T) {
	now := time.Now().UTC()
	auth := &fakeRefreshService{result: &service.AccessTokenResult{
...
func TestAuthHandlerRefresh_MapsServiceErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			serviceErr: service.ErrSessionExpired,
			wantStatus: fiber.StatusUnauthorized,
			wantCode:   "AUTH005",
...
func TestAuthHandlerLogout_RejectsMissingAuthenticationAndSession(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.setUserID {
				app.Use(func(c *fiber.Ctx) error {
					c.Locals(middleware.LocalUserID, uuid.New())
					return c.Next()
				})
			}
			app.Post("/api/auth/logout", handler.Logout)
...
func TestAuthHandlerMe_ReturnsSafeCurrentUser(t *testing.T) {
	...
	auth := &fakeCurrentUserService{user: &domain.User{
		ID:             userID,
		Username:       "admin",
		PasswordHash:   "must-not-be-returned",
		IsLocked:       true,
