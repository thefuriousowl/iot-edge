...
function initialValues(): FormValues {
  ...
  return { name: "", description: "", enabled: true, timezone, mode: "interval", start_local: dateToLocalInput(start, timezone), end_enabled: false, end_local: "", interval_seconds: 60, schedule_unit: "day", schedule_every: 1, times: [{ value: "08:00" }], weekdays: [1, 2, 3, 4, 5] };
}
...
function requestFromValues(values: FormValues, tagIDs: string[]): SaveDataLoggerRequest {
  ...
  return { name: values.name.trim(), description: values.description.trim() || null, enabled: values.enabled, timezone: values.timezone, mode: values.mode, start_at: start.toISOString(), end_at: end?.toISOString() ?? null, config, tag_ids: tagIDs };
}
