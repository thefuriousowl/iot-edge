  ...
  return (
    <section className="device-workspace">
      <header>
        ...
        <button type="button" className="is-primary" onClick={() => { setEditingDevice(null); setShowDeviceForm(true); }}><Plus size={17} /> Add device</button>
      </header>
      {error && <div className="device-workspace-alert" role="alert"><AlertTriangle size={18} /><span>{error}</span><button type="button" onClick={() => setError(null)}><X size={16} /></button></div>}
        ...
        <div className="device-stack">{devices.map((entity) => (
          <article key={entity.id} className="device-card">
            <div className="device-row">
              <button type="button" className="device-expand" onClick={() => void toggleDevice(entity.id)}>{expanded === entity.id ? <ChevronDown /> : <ChevronRight />}<Cpu /><span><strong>{entity.name}</strong><small>Modbus Unit {entity.config.unit_id} · {entity.config.poll_interval_ms} ms</small></span></button>
              <div><span className={entity.enabled ? "is-enabled" : "is-paused"}>{entity.enabled ? "Enabled" : "Paused"}</span><button type="button" aria-label={`Edit ${entity.name}`} onClick={() => { setEditingDevice(entity); setShowDeviceForm(true); }}><Pencil size={17} /></button><button type="button" aria-label={`Delete ${entity.name}`} onClick={() => void removeDevice(entity)}><Trash2 size={17} /></button></div>
            </div>
            {expanded === entity.id && <div className="datasource-panel">
              <div className="datasource-heading"><div><strong>Datasources</strong><small>{datasources[entity.id]?.length ?? 0} configured</small></div><button type="button" onClick={() => setDatasourceFormFor(entity.id)}><Plus size={16} /> Add datasource</button></div>
...
function DeviceForm({ vgatewayId, initial, onCancel, onSaved, onError }: { vgatewayId: string; initial: Device | null; onCancel: () => void; onSaved: (device: Device) => void; onError: (message: string) => void }) {
  const [pending, setPending] = useState(false);
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); const data = new FormData(event.currentTarget); setPending(true);
    try { const input = { name: String(data.get("name")), description: String(data.get("description") || "") || null, enabled: data.get("enabled") === "on", config: { unit_id: Number(data.get("unit_id")), poll_interval_ms: Number(data.get("poll_interval_ms")) } }; onSaved(initial ? await updateDevice(initial.id, input) : await createDevice(vgatewayId, { ...input, type: "modbus_device" })); }
    catch (caught) { onError(errorMessage(caught)); } finally { setPending(false); }
  }
  return <form className="topology-form" onSubmit={(event) => void submit(event)}><label>Name<input name="name" required maxLength={100} defaultValue={initial?.name} /></label><label>Unit ID<input name="unit_id" type="number" min="0" max="255" defaultValue={initial?.config.unit_id ?? 1} required /></label><label>Poll interval (ms)<input name="poll_interval_ms" type="number" min="100" defaultValue={initial?.config.poll_interval_ms ?? 1000} required /></label><label className="is-wide">Description<input name="description" defaultValue={initial?.description ?? ""} /></label><label className="topology-toggle"><input name="enabled" type="checkbox" defaultChecked={initial?.enabled ?? true} /> Enabled</label><div><button type="button" onClick={onCancel}>Cancel</button><button type="submit" className="is-primary" disabled={pending}>{pending ? "Saving…" : initial ? "Update device" : "Save device"}</button></div></form>;
}

function DatasourceForm({ deviceId, initial = null, onCancel, onCreated, onError }: { deviceId: string; initial?: Datasource | null; onCancel: () => void; onCreated: (datasource: Datasource) => void; onError: (message: string) => void }) {
  const [pending, setPending] = useState<"preview" | "save" | null>(null); const [sample, setSample] = useState<DatasourceSample | null>(null);
  function request(form: HTMLFormElement): CreateDatasourceRequest { const data = new FormData(form); return { name: String(data.get("name")), type: "modbus_read", description: String(data.get("description") || "") || null, enabled: data.get("enabled") === "on", config: { function_code: Number(data.get("function_code")) as 1 | 2 | 3 | 4, start_address: Number(data.get("start_address")), quantity: Number(data.get("quantity")), ...(data.get("poll_interval_ms") ? { poll_interval_ms: Number(data.get("poll_interval_ms")) } : {}) } }; }
  async function preview(form: HTMLFormElement) { setPending("preview"); try { setSample(await previewDatasource(deviceId, request(form))); } catch (caught) { onError(errorMessage(caught)); } finally { setPending(null); } }
  async function submit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); setPending("save"); try { const input = request(event.currentTarget); onCreated(initial ? await updateDatasource(initial.id, { name: input.name, description: input.description, enabled: input.enabled, config: input.config }) : await createDatasource(deviceId, input)); } catch (caught) { onError(errorMessage(caught)); } finally { setPending(null); } }
  return <form className="topology-form datasource-form" onSubmit={(event) => void submit(event)}><label>Name<input name="name" required maxLength={100} defaultValue={initial?.name} /></label><label>Function<select name="function_code" defaultValue={initial?.config.function_code ?? 3}><option value="1">01 · Coils</option><option value="2">02 · Discrete inputs</option><option value="3">03 · Holding registers</option><option value="4">04 · Input registers</option></select></label><label>Start address<input name="start_address" type="number" min="0" max="65535" defaultValue={initial?.config.start_address ?? 0} required /></label><label>Quantity<input name="quantity" type="number" min="1" max="2000" defaultValue={initial?.config.quantity ?? 1} required /></label><label>Interval override<input name="poll_interval_ms" type="number" min="100" placeholder="Inherit" defaultValue={initial?.config.poll_interval_ms ?? ""} /></label><label>Description<input name="description" defaultValue={initial?.description ?? ""} /></label><label className="topology-toggle"><input name="enabled" type="checkbox" defaultChecked={initial?.enabled ?? true} /> Enabled</label><div><button type="button" onClick={onCancel}>Cancel</button><button type="button" disabled={pending !== null} onClick={(event) => void preview(event.currentTarget.form!)}><RefreshCw size={15} /> {pending === "preview" ? "Reading…" : "Preview"}</button><button type="submit" className="is-primary" disabled={pending !== null}>{pending === "save" ? "Saving…" : initial ? "Update" : "Save"}</button></div>{sample && <SampleView sample={sample} />}</form>;
}

function DatasourceRow({ datasource, monitoringEnabled, onUpdated, onDelete, onError }: { datasource: Datasource; monitoringEnabled: boolean; onUpdated: (datasource: Datasource) => void; onDelete: () => void; onError: (message: string) => void }) {
