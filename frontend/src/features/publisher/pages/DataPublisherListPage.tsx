...
import { Activity, ChevronLeft, ChevronRight, CircleAlert, CircleStop, LoaderCircle, Play, RadioTower, RefreshCw, RotateCcw, Search, Server, Settings2, Trash2 } from "lucide-react";
...
function httpListenerLabel(state: DataPublisher["runtime"]["state"]): string {
  if (state === "running") return "Listener running";
  if (state === "starting") return "Listener starting";
  if (state === "stopping") return "Listener stopping";
  if (state === "error") return "Listener unavailable";
  return "Listener stopped";
}

function mqttConnectionLabel(item: DataPublisher): string {
  return item.runtime.state === "running" && item.runtime.connected ? "Connected" : "Disconnected";
}

function httpActiveConnections(item: DataPublisher): number {
  return item.runtime.state === "running" ? item.runtime.active_connections ?? 0 : 0;
}
...
function DataPublisherListPage() {
  ...
  const publisherType = rawType === "mqtt" || rawType === "http_server" ? rawType : undefined;
  ...
  useEffect(() => {
    ...
    void listDataPublishers({ type: publisherType, enabled, search: search || undefined, page, per_page: perPage }, controller.signal).then((response) => {
  ...
  }, [enabled, page, publisherType, refresh, search]);
  ...
  function configurePath(item: DataPublisher): string | null {
    if (item.type === "mqtt") return `/data-publishers/${encodeURIComponent(item.id)}/mqtt`;
    if (item.type === "http_server") return `/data-publishers/${encodeURIComponent(item.id)}/http-server`;
    return null;
  }

  return <VGatewayShell breadcrumb={<>Dashboard <span>/</span> <strong>Data Publishers</strong></>}>
    <div className="plugin-content">
      <section className="plugin-heading"><div><p className="plugin-eyebrow">Core egress</p><h1>Data Publishers</h1><p>Expose existing Core Tag and Plugin-output snapshots without creating additional acquisition requests.</p></div><div className="publisher-heading-actions"><Link className="plugin-primary-link" to="/data-publishers/new/http-server"><Server size={18} />New HTTP Server</Link><Link className="plugin-primary-link is-secondary" to="/data-publishers/new/mqtt"><RadioTower size={18} />New MQTT Publisher</Link></div></section>
      ...
      <section className="plugin-toolbar" aria-label="Publisher filters"><form role="search" onSubmit={submitSearch}><Search size={18} /><label className="sr-only" htmlFor="publisher-search">Search Publishers</label><input id="publisher-search" name="search" type="search" key={search} defaultValue={search} placeholder="Search Data Publishers…" /><button type="submit">Search</button></form><select aria-label="Publisher transport" value={publisherType ?? "all"} onChange={(event) => updateFilter("type", event.target.value === "all" ? undefined : event.target.value)}><option value="all">All transports</option><option value="http_server">HTTP Server</option><option value="mqtt">MQTT</option></select><select aria-label="Desired state" value={rawEnabled ?? "all"} onChange={(event) => updateFilter("enabled", event.target.value === "all" ? undefined : event.target.value)}><option value="all">All desired states</option><option value="true">Enabled</option><option value="false">Disabled</option></select><button type="button" onClick={() => setRefresh((value) => value + 1)}><RefreshCw className={state === "loading" ? "is-spinning" : ""} size={17} /> Refresh</button></section>
      {message && state !== "error" && <div className="plugin-alert" role="alert"><CircleAlert size={18} />{message}</div>}
      <section className="plugin-list" aria-label="Data Publishers">
        {state === "loading" && <div className="plugin-state" role="status"><LoaderCircle className="is-spinning" /><strong>Loading Data Publishers…</strong></div>}
        ...
        {state === "ready" && publishers.length === 0 && <div className="plugin-state"><Server /><strong>No Data Publishers found</strong><span>Create a disabled HTTP Server or MQTT definition, verify it, then enable the runtime.</span><Link to="/data-publishers/new/http-server">New HTTP Server</Link></div>}
        {state === "ready" && publishers.length > 0 && <><div className="plugin-table-scroll"><table className="publisher-table"><thead><tr><th>Publisher</th><th>Desired</th><th>Runtime</th><th>Activity</th><th>Transport</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{publishers.map((item) => { const configure = configurePath(item); return <tr key={item.id}><td data-label="Publisher"><strong>{item.name}</strong><small>{item.type === "http_server" ? "HTTP Server" : item.type === "mqtt" ? "MQTT" : "Unsupported transport"} · {item.source_count} sources · Config v{item.config_version}</small></td><td data-label="Desired"><span className={`plugin-desired is-${item.enabled ? "enabled" : "disabled"}`}>{item.enabled ? "Enabled" : "Disabled"}</span></td><td data-label="Runtime"><span className={`plugin-runtime is-${item.runtime.state}`}><i />{item.runtime.state}</span>{(item.runtime.last_error || item.runtime.transport_error) && <small className="plugin-runtime-error">{item.runtime.last_error || item.runtime.transport_error}</small>}</td><td data-label="Activity">{item.type === "http_server" ? <>{item.runtime.external_request_count ?? 0} requests<small>{item.runtime.rejected_request_count ?? 0} rejected · {item.runtime.publish_count} snapshot updates</small></> : <>{item.runtime.delivery_count} delivered<small>{item.runtime.transport_drop_count} dropped · {item.runtime.delivery_failure_count} failed</small></>}</td><td data-label="Transport">{item.type === "http_server" ? <>{httpActiveConnections(item)} active<small>{httpListenerLabel(item.runtime.state)}</small></> : <>{mqttConnectionLabel(item)}<small>{item.runtime.reconnect_count} reconnects</small></>}</td><td data-label="Actions"><div className="plugin-row-actions">{configure && <Link aria-label={`Configure ${item.name}`} to={configure}><Settings2 size={16} /><span>Configure</span></Link>}{item.enabled ? <button aria-label={`Disable ${item.name}`} disabled={actionID === item.id} onClick={() => void runAction(item, "disable")}><CircleStop size={16} /><span>Disable</span></button> : <button aria-label={`Enable ${item.name}`} disabled={actionID === item.id} onClick={() => void runAction(item, "enable")}><Play size={16} /><span>Enable</span></button>}<button aria-label={`Restart ${item.name}`} disabled={!item.enabled || actionID === item.id} onClick={() => void runAction(item, "restart")}><RotateCcw size={16} /><span>Restart</span></button><button className="is-danger" aria-label={`Delete ${item.name}`} disabled={actionID === item.id} onClick={() => void runAction(item, "delete")}><Trash2 size={16} /><span>Delete</span></button></div></td></tr>; })}</tbody></table></div><footer className="plugin-pagination"><p>Showing {publishers.length} of {pagination.total}</p><div><button aria-label="Previous page" disabled={page <= 1} onClick={() => updateFilter("page", String(page - 1))}><ChevronLeft /></button><span>{pagination.page}</span><button aria-label="Next page" disabled={page >= pagination.total_pages} onClick={() => updateFilter("page", String(page + 1))}><ChevronRight /></button></div></footer></>}
      </section>
    </div>
  </VGatewayShell>;
}
